...
async def facebook_prepare_replies(
    ...
    # Determine which pages to include. If multiple page_ids were provided, use them; otherwise, use a single page_id.
...
async def facebook_post_replies(
    ...
    user_id = current_user.get("id")
    # Determine which pages to include when posting replies
    selected_pages: Optional[list[str]] = None
    if page_ids:
        selected_pages = page_ids
    elif page_id:
