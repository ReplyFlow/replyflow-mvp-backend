...
# Define the scopes that your application will request from Facebook during
# the OAuth flow. Each permission listed here must be approved for your
# app through the Facebook App Review process in order for production
# users to grant them. The previous implementation included
# `pages_read_user_engagement` which is not a valid permission and caused
# login failures. The scopes below cover reading engagement data,
# managing posts and engagement on Pages, and reading a list of pages the
# user manages. If you need additional capabilities (such as managing
# ads or reading user‑generated content), you should add the corresponding
# permissions here and complete the appropriate Facebook review steps.
FACEBOOK_SCOPES = [
    # Allows your app to like and reply to comments on behalf of a Page as well
    # as delete unwanted comments from the Page. Required for replying to
    # comments and posts via the API.
    "pages_manage_engagement",

    # Allows your app to create, edit and delete your Page posts. Needed to
    # publish replies or posts through the API.
    "pages_manage_posts",

    # Allows your app to read content (posts, photos, videos, events) posted
    # by the Page as well as follower data and metadata. This replaces the
    # deprecated `pages_read_user_engagement` permission.
    "pages_read_engagement",

    # Allows your app to read the list of Pages that a person manages. This
    # dependency is required when requesting other Page permissions.
    "pages_show_list",
]
...
async def facebook_callback(code: str, state: Optional[str] = None) -> HTMLResponse:
    ...
    # Determine where to redirect the user after the OAuth flow completes. Use
    # the FRONTEND_DASHBOARD_URL environment variable if available, falling
    # back to the DASHBOARD_URL for backwards compatibility. If neither is
    # set, default to the root path. This ensures that a sensible
    # destination exists even if the configuration is incomplete.
    frontend_dashboard = os.getenv("FRONTEND_DASHBOARD_URL") or os.getenv("DASHBOARD_URL") or "/"

    # Render a small HTML page that attempts to notify the opener window
    # that the Facebook connection succeeded, then closes the popup. If
    # the page is not opened in a popup (i.e., window.opener is null),
    # perform a full redirect back to the dashboard instead of relying
    # on Facebook's own /dialog/close URL. This avoids the "Can't Load URL"
    # error when the Facebook app's domains are not configured to allow
    # facebook.com as a redirect domain. A <noscript> fallback is
    # provided for browsers with JavaScript disabled.
    html_content = f"""
    <html>
        ...
        <script>
          (function() {{
            try {{
              ...
              console.warn('Unable to notify opener or close window:', e);
            }}
            // Fallback: redirect the current window to the dashboard
            window.location.href = "{frontend_dashboard}";
          }})();
        </script>
        <noscript>
          <h2>Facebook Connected</h2>
          <p>Your Facebook Page has been successfully connected. You can now close this window and return to the dashboard.</p>
        </noscript>
      </body>
    </html>
    """
