        ...
        for token_info in fb_tokens:
            ...
            for m in media:
                ...
                for c in comments[:max_comments]:
                    ...
                    try:
                        if _openai_available and os.getenv("OPENAI_API_KEY"):
                            reply_text = await generate_with_openai(comment_text, tone)
                        else:
...
async def instagram_post_replies(
    ...
    """
    user_id = current_user.get("id")
    # Parse request body for tone, limits and page selection
    selected_pages: Optional[list[str]] = None
    body_data: Dict[str, Any] = {}
    if request and request.method == "POST":
        try:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            for tw in tweets[:max_posts]:
                ...
                try:
                    if _openai_available and os.getenv("OPENAI_API_KEY"):
                        reply_text = await generate_with_openai(tweet_text, tone)
                    else:
...
async def x_prepare_replies(
    ...
    """
    user_id = current_user.get("id")
    # Parse JSON body for tone and max_posts if provided
    if request and request.method == "POST":
        try:
...
async def x_post_replies(
    ...
    """
    user_id = current_user.get("id")
    # Parse body or query params for tone and max_posts
    if request and request.method == "POST":
        try:
            body = await request.json()
        except Exception:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            for child in children:
                ...
                try:
                    if _openai_available and os.getenv("OPENAI_API_KEY"):
                        reply_text = await generate_with_openai(comment_body, tone)
                    else:
...
async def reddit_prepare_replies(
    ...
    """
    user_id = current_user.get("id")
    # Parse body for tone and max_posts
    if request and request.method == "POST":
        try:
            body = await request.json()
        except Exception:
