...
FACEBOOK_SCOPES = [
    ...
    # and view the list of Pages. `pages_read_user_content` allows reading
    # comments on posts created by the Page or others.
...
async def facebook_prepare_replies(
    ...
    """
    # Pass the current user's ID into the helper so it only fetches comments
    # for Pages that belong to this user.
...
async def facebook_login(token: Optional[str] = None) -> RedirectResponse:
    ...
    if not user_id:
        frontend_login = os.getenv("FRONTEND_LOGIN_URL") or "/login.html"
        return RedirectResponse(frontend_login)
    # Create a unique state value and record the mapping to the user ID
    state = uuid.uuid4().hex
    pending_facebook_states[state] = user_id
    params = {
        "client_id": client_id,
