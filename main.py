...
async def bluesky_oauth_start(
    ...
    This endpoint generates a PKCE code verifier/challenge and a state value,
    stores them in memory, and then redirects the user to the PDS's
    authorization endpoint.  The user must be authenticated within
    ReplyFlow (have a current_user); otherwise they are redirected to
    the login page.
    ...
    """
    # Verify that the user is logged in.  If not, send them to the login page.
    ...
    # Persist in-memory for later retrieval during callback.
    BLUESKY_OAUTH_STATES[state] = {"code_verifier": code_verifier, "user_id": user_id}
    # Determine base URL for constructing client_id and redirect_uri.  Use the
    # same logic as the metadata endpoint.
    base_url = os.getenv("BLUESKY_METADATA_BASE_URL")
    if not base_url:
        scheme = request.url.scheme
        host = request.url.hostname
    ...
    redirect_uri = os.getenv("BLUESKY_REDIRECT_URI") or f"{base_url}/bluesky/oauth/callback"
    scope = os.getenv("BLUESKY_SCOPE", "transition:generic")
    # Determine the PDS authorization server.  Default to bsky.social.  In
    # production you may derive this per user based on their handle's DID.
    pds_host = os.getenv("BLUESKY_PDS_HOST", "https://bsky.social")
    # Build the authorization URL.  Use urllib.parse.quote to ensure proper
    # encoding of query parameters.  We do not include DPoP here; it is only
...
async def bluesky_oauth_callback(
    ...
    if not code or not user_id:
        ...
        """
        return HTMLResponse(content=error_html, status_code=400)
    # Perform the token exchange.  In a real implementation you would send
    # an HTTP POST to the token endpoint with the code and verifier and
    # parse the returned JSON.  Since external requests are disabled here,
    # we'll generate placeholder access and refresh tokens.
