...
async def facebook_prepare_replies(
    ...
    # Determine selected pages based on query parameters or JSON body. If the request is POST and includes
    # page selection fields in the body, parse them. For GET requests, rely on query parameters only.
    ...
    if request and request.method == "POST":
        try:
            ...
            body_data = {}
        # Read page_ids and page_id from body if provided
        body_page_ids = body_data.get("page_ids") if isinstance(body_data, dict) else None
        body_page_id = body_data.get("page_id") if isinstance(body_data, dict) else None
    ...
    # If not found in body, fall back to query parameters
...
async def facebook_post_replies(
    ...
    user_id = current_user.get("id")
    # Determine which pages to include when posting replies. Parse body first for POST requests.
    selected_pages: Optional[list[str]] = None
    body_data: Dict[str, Any] = {}
    if request and request.method == "POST":
        try:
