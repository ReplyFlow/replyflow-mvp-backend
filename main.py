...
def generate_intercom_jwt(user_id: str = Query(...)) -> Dict[str, str]:
    """Generate a signed JWT for the Intercom Messenger.

    The `user_id` must uniquely identify the currently logged in user. It
    becomes the `sub` claim in the JWT. Intercom uses this to match the
    signed token to your customer record. The token is valid for 24 hours.
