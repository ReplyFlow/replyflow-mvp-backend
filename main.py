...
# -----------------------------------------------------------------------------
# Facebook OAuth session tracking
#
# When initiating the Facebook login flow, we need to associate the OAuth
# "state" parameter with the current authenticated session token. This allows
# us to look up the user after Facebook redirects back to our callback. The
# mapping below stores state -> session_token for pending login attempts.
...
async def facebook_login(request: Request) -> RedirectResponse:
    """Begin the Facebook OAuth flow for an authenticated user.

    The frontend should include the current session token as a query parameter
    (e.g. /facebook/login?token=abc123). We validate this token against the
    in‑memory session store. If it is invalid or missing, we redirect the
    browser to the frontend login page to prompt re‑authentication.

    Upon successful validation, we generate a unique state value, persist
    the mapping of state -> session token, and then redirect to Facebook's
    authorization dialog with the state included. When Facebook redirects
    back to our callback, we will use the state to associate the OAuth
    response with the correct user session.
    """
    ...
    # Determine the frontend login URL. This may be explicitly set via environment
    # variables; otherwise attempt to derive it from the Referer or Origin header.
    frontend_login_url = os.getenv("FRONTEND_LOGIN_URL")
    if not frontend_login_url:
        referer = request.headers.get("Referer") or request.headers.get("Origin")
        if referer:
            try:
                parsed = urllib.parse.urlparse(referer)
                base = f"{parsed.scheme}://{parsed.netloc}"
                frontend_login_url = f"{base}/login.html"
            except Exception:
                frontend_login_url = "/login.html"
        else:
            frontend_login_url = "/login.html"
    # Extract the session token either from the query string or Authorization header
    token_param = request.query_params.get("token")
    auth_header = request.headers.get("Authorization")
    token_header = None
    if auth_header and auth_header.startswith("Bearer "):
        token_header = auth_header.split(" ", 1)[1].strip()
    session_token = token_param or token_header
    # Validate the session token
    user_id = session_store.get_user_id(session_token) if session_token else None
    if not user_id:
        # If no valid session is found, redirect the user to the frontend login page.
        return RedirectResponse(frontend_login_url)
    # Generate a state parameter and remember which session initiated it
    state = uuid.uuid4().hex
    pending_facebook_states[state] = session_token
...
async def facebook_callback(code: str, state: Optional[str] = None) -> HTMLResponse:
    """Handle the OAuth callback from Facebook, exchange the code, and notify the frontend.

    Facebook redirects the user back to this endpoint with a short‑lived
    authorization code and the state value we supplied. We exchange the code
    for a user access token, then fetch the list of Pages the user manages
    in order to obtain a Page access token. Tokens are persisted using the
    global token_store. Finally, we return a small HTML page that notifies
    the parent window (if one exists) that Facebook has been connected and
    either closes the popup or redirects to the dashboard.
    """
    ...
    # Retrieve the session token associated with this state, if any.
    session_token = None
    if state:
        session_token = pending_facebook_states.pop(state, None)
    ...
    async with httpx.AsyncClient() as client:
        ...
        # user must have the required permissions (pages_manage_engagement).
    ...
    # Persist tokens for each page the user manages. In a production system
    # you would associate these tokens with the authenticated user (session_token).
    ...
    # Determine the dashboard URL. Try environment variables first, then fallback to root.
    frontend_dashboard_url = os.getenv("FRONTEND_DASHBOARD_URL") or "/"
    # Build an HTML response that notifies the opener (parent) window that
    # Facebook has been connected. If this callback is opened in a popup
    # window, we use window.opener.postMessage to inform the original
    # dashboard to refresh its state. Otherwise, we simply redirect the
    # current page to the dashboard URL.
    html_content = f"""
    <html>
    <head>
    ...
    <body>
        ...
        <script>
            (function() {{
                var dashboardUrl = "{frontend_dashboard_url}";
                if (window.opener) {{
                    // Notify the opener that Facebook was connected
                    window.opener.postMessage("facebook_connected", "*");
                    // Attempt to close the popup window
                    window.close();
                }} else {{
                    // Fallback: redirect the current page to the dashboard
