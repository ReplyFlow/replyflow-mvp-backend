...
async def facebook_callback(code: str, state: Optional[str] = None) -> HTMLResponse:
    ...
    async with httpx.AsyncClient() as client:
        ...
        # Fallback: if no pages are returned (common with New Page Experience or Commerce pages),
        # attempt to fetch a single page token using an explicit page ID provided via env var.
        # This allows users to connect pages that are not listed in /me/accounts. The
        # environment variable FACEBOOK_PAGE_ID should contain the ID of the page to connect.
        if (not pages_data or not pages_data.get("data")):
            fallback_page_id = os.getenv("FACEBOOK_PAGE_ID")
            if fallback_page_id:
                try:
                    page_resp = await client.get(
                        f"https://graph.facebook.com/v18.0/{fallback_page_id}",
                        params={"access_token": access_token, "fields": "id,name,access_token"},
                        timeout=10,
                    )
