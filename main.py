...
# ----------------------------------------------------------------------------
# LinkedIn OAuth state storage
#
# When initiating an OAuth authorization request for LinkedIn, we generate a
# random ``state`` value tied to the current user's session. The state
# parameter is sent to LinkedIn and returned back to us on the callback.
# Storing the mapping between state and user_id prevents CSRF attacks and
# lets us look up the user when the authorization code is returned. The
# dictionary maps the state to the user_id who initiated the flow. Since the
# states only live in memory, restarting the server will invalidate in‑flight
# authorizations. In a production deployment, persist this mapping in a
# database or cache.
LINKEDIN_OAUTH_STATES: Dict[str, str] = {}
...
async def linkedin_post_replies(
    ...
    This endpoint fetches comments (via the generic prepare function) and
    publishes a new post on the authenticated member's feed for each
    generated reply. It uses the LinkedIn REST Posts API and requires that
    the user has already completed the LinkedIn OAuth flow via
    ``/linkedin/oauth/start`` and ``/linkedin/oauth/callback``. The access
    token and author URN are retrieved from the token store. If posting
    fails or no tokens are available, the function falls back to the
    generic implementation which returns zero posts made.
    """
    # First, generate the replies using the generic prepare function. This will
    # enforce tone, free plan limits and other business rules. We ignore
    # max_comments here because LinkedIn does not support comment replies via
    # public API; instead we create new top‑level posts for each generated
    # reply.
    user_id = current_user.get("id")
    # Allow overrides via JSON body or query parameters similar to the generic
    # prepare function. This code mirrors the parsing logic used in other
    # post endpoints (e.g. x_post_replies) to support overrides.
    if request and request.method == "POST":
        try:
            body = await request.json()
        except Exception:
    ...
    if request:
        ...
        if qp_mt:
            try:
                ...
                pass
    # Generate replies via the generic prepare helper. Although LinkedIn
    # comments are not currently fetched via the API (due to missing public
    # endpoints), this call allows tone and plan limits to be applied
    # consistently across providers. The returned structure is a list of
    # dictionaries with at least a ``reply`` key containing the reply text.
    prepared = await _generic_prepare(
        provider="linkedin",
        tone=tone,
        max_posts=max_posts,
        max_comments=max_comments,
    ...
    # Look up the LinkedIn access token and author URN for this user from
    # the token store. The access token is stored in the page_access_token
    # field and the author URN in the user_access_token field. If no token
    # exists, fall back to the generic post implementation.
    tokens = token_store.list_tokens(user_id, "linkedin")
    if not tokens:
        # No tokens stored; cannot post. Return result of generic handler.
        return {"replies_posted": 0}
    # Use the first stored token entry. Future enhancements could support
    # multiple connections per user.
    token_entry = tokens[0]
    access_token = token_entry.get("page_access_token") or token_entry.get("user_access_token")
    # The author URN is stored in the user_access_token field. If absent,
    # attempt to derive from ig_account_id or default to None.
    author_urn = token_entry.get("user_access_token") or token_entry.get("ig_account_id")
    if not access_token or not author_urn:
        return {"replies_posted": 0}
    # Build headers required by the LinkedIn REST Posts API. LinkedIn
    # recommends specifying the version header (YYYYMM format) and the
    # RestLi protocol version. Without these headers, API calls may fail.
    headers = {
        "Authorization": f"Bearer {access_token}",
        "LinkedIn-Version": os.getenv("LINKEDIN_API_VERSION", "202507"),
        "X-Restli-Protocol-Version": "2.0.0",
        "Content-Type": "application/json",
    }
    # Post each reply as a new feed update. We ignore blacklisted items (as
    # determined by the generic prepare helper) and skip entries with
    # missing reply text. LinkedIn imposes a 1300 character limit on
    # commentary text; truncate longer replies accordingly.
    posted_count = 0
    for item in prepared:
        # Skip blacklisted entries or those without a reply field
        if item.get("blacklisted"):
            continue
        reply_text = item.get("reply") or item.get("commentary") or item.get("text")
        if not reply_text:
            continue
        # Truncate commentary to LinkedIn's 1300 character limit
        commentary = reply_text.strip()
        if len(commentary) > 1300:
            commentary = commentary[:1297] + "..."
        payload = {
            "author": author_urn,
        ...
        try:
            ...
            # A 201 Created response indicates success. Some accounts may
            # return 200 OK on success as well.
            ...
            continue
    return {"replies_posted": posted_count}
...
@app.get("/linkedin/oauth/start")
async def linkedin_oauth_start(
    token: Optional[str] = None,
    request: Request = None,
) -> RedirectResponse:
    """Initiate the LinkedIn OAuth authorization flow.

    This endpoint constructs the LinkedIn OAuth 2.0 authorization URL and
    redirects the user to it. A unique ``state`` value is generated to
    prevent CSRF attacks and is stored in ``LINKEDIN_OAUTH_STATES`` along with
    the current user's ID. The user's session token must be provided via
    the ``token`` query parameter so that we can determine who is initiating
    the OAuth flow. The client ID and redirect URI must be configured via
    environment variables ``LINKEDIN_CLIENT_ID`` and ``LINKEDIN_REDIRECT_URI``.
    ...
    Returns:
        A RedirectResponse sending the user to LinkedIn's OAuth authorization
        endpoint.
    """
    session_token = token
    if not session_token:
    ...
    if not user_id:
        login_url = os.getenv("FRONTEND_LOGIN_URL") or "/login.html"
        return RedirectResponse(login_url)
    # Generate a random state and store mapping to user_id
    state = secrets.token_urlsafe(16)
    LINKEDIN_OAUTH_STATES[state] = user_id
    # Resolve client ID
    client_id = os.getenv("LINKEDIN_CLIENT_ID")
    if not client_id:
        error_html = """
        <html><body>
          <p>Missing LINKEDIN_CLIENT_ID environment variable. Please set it in the server configuration.</p>
          <script> window.close(); </script>
        </body></html>
        """
        return HTMLResponse(error_html, status_code=500)
    # Determine the redirect URI; fallback to request host if not specified
    ...
    if not redirect_uri:
        ...
        port = request.url.port if request else None
        if port and ((scheme == "http" and port != 80) or (scheme == "https" and port != 443)):
            redirect_uri = f"{scheme}://{host}:{port}/linkedin/oauth/callback"
        else:
            redirect_uri = f"{scheme}://{host}/linkedin/oauth/callback"
    # LinkedIn scopes: r_liteprofile is required to fetch the member ID; w_member_social
    # grants ability to publish posts on behalf of the member. These scopes are
    # space‑separated and must be URL‑encoded. Additional scopes can be added
    # via the LINKEDIN_SCOPE environment variable if needed.
    scope_env = os.getenv("LINKEDIN_SCOPE")
    scope = scope_env or "r_liteprofile w_member_social"
    params = {
        "response_type": "code",
        "client_id": client_id,
        "redirect_uri": redirect_uri,
...
async def linkedin_oauth_callback(
    ...
    """Handle the LinkedIn OAuth callback and exchange the code for a token.

    This endpoint receives the authorization ``code`` and ``state`` from
    LinkedIn. It validates the state, exchanges the code for an access
    token via LinkedIn's token endpoint, retrieves the member's ID using
    the ``/v2/me`` endpoint, stores the token and URN in the token store,
    and notifies the opener window of success. On error, an error page
    with a corresponding postMessage is returned.

    Args:
        code: The authorization code returned by LinkedIn.
        state: The state parameter generated during the start of the flow.
        request: The incoming request (used to derive redirect URI if unset).

    Returns:
        An HTMLResponse containing a script that posts a message back to the
    ...
    if not state or state not in LINKEDIN_OAUTH_STATES:
        ...
        <html><body>
          <p>Invalid or expired OAuth state. Please try connecting again.</p>
          <script>
            if (window.opener) { window.opener.postMessage('linkedin_failed', '*'); }
            window.close();
          </script>
        </body></html>
        """
        return HTMLResponse(content=error_html, status_code=400)
    user_id = LINKEDIN_OAUTH_STATES.pop(state)
    # Ensure we have both a code and user ID
    if not code or not user_id:
        error_html = """
        <html><body>
          <p>Authorization failed or was cancelled.</p>
    ...
    if not redirect_uri:
        ...
        if port and ((scheme == "http" and port != 80) or (scheme == "https" and port != 443)):
            ...
            redirect_uri = f"{scheme}://{host}/linkedin/oauth/callback"
    # Exchange the authorization code for an access token. If either
    # client_id or client_secret is missing, we will fall back to a random
    # token so that the flow does not break completely during development.
    ...
    if client_id and client_secret:
        ...
        try:
            ...
            access_token = None
    # Fallback: generate a random token if real exchange fails (for local dev)
    if not access_token:
        access_token = secrets.token_urlsafe(48)
    # Retrieve the member's ID using the access token
    ...
    try:
        ...
        author_urn = None
    # Fallback: generate a dummy URN if retrieval fails
    if not author_urn:
        author_urn = f"urn:li:person:{secrets.token_hex(8)}"
    # Persist the access token and author URN in the token store. We use
    # page_id="self" to mirror other providers. The author URN is stored
    # in the user_access_token field, and ig_account_id is left as None.
    try:
        token_store.save_token(
            user_id=user_id,
            provider="linkedin",
            ...
            page_access_token=access_token,
            ig_account_id=None,
            user_access_token=author_urn,
        )
    except Exception:
        # Ignore storage errors; the UI will still reflect no connection
        pass
    # Return a success page that notifies the opener and closes the popup
    success_html = """
    <html><body>
      <script>
        try {
      ...
      <p>LinkedIn connection complete. You may close this window.</p>
    </body></html>
    """
    return HTMLResponse(content=success_html, status_code=200)
