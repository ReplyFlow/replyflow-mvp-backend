...
# Permissions requested during the Facebook OAuth flow.
#
# These scopes allow the app to read recent posts and comments from pages the
# user manages and generate replies. The minimum set required to fetch
# comments includes pages_read_engagement (read public content) and
# pages_manage_engagement (reply to comments). We also request
# pages_manage_posts and pages_read_user_content so that we can read and
# respond to user‑generated content on page posts. pages_show_list lets
# the app retrieve the list of pages the user manages. See Meta docs for
# details: https://developers.facebook.com/docs/permissions/reference
FACEBOOK_SCOPES = [
