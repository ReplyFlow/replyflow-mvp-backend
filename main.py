...
async def youtube_upload_video(
    ...
    """Upload a video to the user's connected YouTube channel.

    This endpoint accepts a video file along with metadata (title,
    description and privacy status) and uploads it to YouTube on behalf of
    the authenticated user.  The user must have previously connected
    their YouTube account via OAuth.  Prior to uploading, the helper
    ...
    Args:
        ...
        file: The uploaded video file (e.g., MP4).
        current_user: The authenticated user (injected via dependency).
        request: The incoming request (unused but kept for parity with other
            handlers).

    Returns:
        A JSON dictionary indicating success or failure.  On success,
        ``status`` will be ``"success"`` and ``video_id`` will contain
        the ID of the uploaded video.  On failure, ``status`` will be
        ``"error"`` and ``message`` will describe the problem.
    """
    ...
    try:
        ...
        # Ignore refresh errors; we will attempt upload regardless
        pass
    ...
    # Use the first token's page_access_token as the access token
    ...
    # Build the request to YouTube's resumable upload endpoint
...
async def generate_reply(req: GenerateReplyRequest, request: Request) -> GenerateReplyResponse:
    ...
    if auth_header.startswith("Bearer "):
        ...
        if user_id:
            try:
                ...
                user = None
    # Determine user plan and enforce per‑day limits if free
    if user:
        user_plan = (user.get("plan") or "free").strip().lower()
        # If the user is on a free plan (not trial/solo/pro), enforce limits
        if user_plan == "free":
            ...
            # We'll increment after reply generation below
        else:
            # Paid or trial plans: no limits; all tones accepted
            pass
    else:
    ...
    # If we have a valid user on a free plan and have not exceeded the limit,
    # increment their daily usage count by one
