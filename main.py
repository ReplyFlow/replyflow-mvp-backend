...
async def send_beta_welcome_email_endpoint(request: BetaWelcomeRequest) -> Dict[str, str]:
    ...
    Args:
        request: A BetaWelcomeRequest containing the recipient's email and
            optional name.

    Returns:
        A dict indicating success.  If an error occurs, an HTTPException is
        raised with status 500 and the error message.
    """
    try:
...
# -----------------------------------------------------------------------------
# Beta registration and welcome email dispatch
#
# In addition to serving the beta sign‑up page, the backend now includes a
# `POST /beta/register` endpoint.  This endpoint accepts a JSON payload
# containing the new tester's email address and an optional name.  When called,
# it validates the request, checks that the beta sign‑up flow is enabled via
# feature flag, and immediately sends the welcome email using the existing
# `send_beta_welcome_email` helper.  This endpoint can be used directly by
# client‑side code (e.g. on a custom sign‑up form) or configured as a webhook
# target in external form services like Zoho Forms.  If the environment
# variables required for email sending are not configured, or if the beta sign‑up
# flow is disabled, the endpoint will return an appropriate HTTP error.
...
async def beta_register(request: BetaRegistrationRequest) -> Dict[str, str]:
    ...
    Args:
        request: A BetaRegistrationRequest containing the tester's email and
            optional name.

    Returns:
        A dict indicating success.  If an error occurs, an HTTPException is
        raised with status 403 or 500 depending on the cause.
    """
    # Ensure the beta sign‑up flow is enabled via feature flag
...
async def feature_flags() -> Dict[str, bool]:
    ...
    This endpoint allows the frontend to determine whether certain
    experimental or beta features should be visible. It reads environment
    variables at startup and returns their boolean values.

    Returns:
        A mapping of feature names to their enabled state. Currently includes:
        - video_upload: whether the YouTube video upload form should be shown.
        - beta_signup: whether the beta sign‑up link and page should be available.
    """
    return {
