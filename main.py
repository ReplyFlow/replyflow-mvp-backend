...
# -----------------------------------------------------------------------------
# Frontend configuration and optional features
#
# The ReplyFlow welcome email references URLs for logging in and optional
# functionality (browser extension, PWA install) that may vary between
# deployments.  To avoid hard‑coding a specific domain, you can set the
# FRONTEND_BASE_URL environment variable to the root URL where your
# frontend is hosted (for example, "https://replyflowapp.com" or the
# Render-hosted backend URL if serving static pages from the API server).
#
# Additionally, the availability of certain features can be toggled via
# environment variables.  If ENABLE_BROWSER_EXTENSION is set to "true",
# step 3 of the welcome email will describe how to install the extension;
# otherwise it will mention that the extension is coming soon.  Similarly,
# ENABLE_PWA controls whether the PWA installation instructions are shown in
# the welcome email.

# Configure the base URL for the ReplyFlow front‑end.  Use an environment
# variable to allow overriding in different environments.  The default
# value points to the production domain (replyflowapp.com).  If you
# previously set this to the Render backend URL, update your
# environment or change the default below to your desired domain.
FRONTEND_BASE_URL = os.getenv("FRONTEND_BASE_URL", "https://replyflowapp.com")

ENABLE_BROWSER_EXTENSION = os.getenv("ENABLE_BROWSER_EXTENSION", "false").lower() == "true"
...
def _compose_beta_welcome_body(name: str | None = None) -> str:
    ...
    The welcome email introduces new beta testers to ReplyFlow and explains
    how to get started.  It dynamically references environment variables
    to construct URLs and conditionally includes optional steps based on
    feature flags.  This allows the same codebase to be deployed in
    different environments without rewriting the email content.

    Args:
        name: Optional display name of the recipient.  If provided, it
            personalizes the greeting.  When omitted or empty, a generic
            greeting is used.
    ...
    # Build the login URL from the configured base.  Ensure there is no
    # trailing slash to avoid duplicate slashes when appending the path.
    # Fallback to the public domain if the environment variable is unset or blank.
    base = FRONTEND_BASE_URL.rstrip('/') or "https://replyflowapp.com"
    login_url = f"{base}/login.html"

    # Step 1: login instructions.  Use the computed login_url.
    step1 = (
        f"1. Logging in: Visit {login_url} and click “Login”. "
        "Use the same email you signed up with; you'll be prompted to set a password or request a "
        "magic link."
    )
    # Step 2: support channels remain constant.
    step2 = (
    ...
    # Step 3: browser extension installation or placeholder message based on flag.
    if ENABLE_BROWSER_EXTENSION:
        step3 = (
        ...
        step3 = (
            "3. Browser extension: Coming soon! We'll let you know once it's available in the "
            "Chrome Web Store."
        )
    # Step 4: PWA installation instructions or placeholder based on flag.
    if ENABLE_PWA:
        step4 = (
        ...
        step4 = (
            "4. Progressive Web App: Coming soon! We'll notify you when offline and install "
            "capabilities are available."
        )
    # Assemble the full body with conditional steps.
    return (
        f"{greeting}\n\n"
...
def _register_beta_endpoints(app: FastAPI) -> None:
    ...
    async def beta_register(request: BetaRegistrationRequest) -> Dict[str, str]:
        ...
        try:
            # Record this email as belonging to a beta tester.  This allows the
            # signup flow to tag the resulting user account with the
            # ``beta_user`` flag when the user later registers via /signup.
            beta_user_store.add_beta_email(email)
...
# Basic account pages
#
# To provide a persistent login experience, we expose simple HTML pages for
# user authentication.  These pages live in the project root as
# ``login.html`` and ``signup.html``.  When requested, the server will
# attempt to serve the corresponding file.  If the file is missing, a
# fallback message is returned instead.  The frontend pages handle
# communicating with the existing /login and /signup API endpoints.

@app.get("/login.html", response_class=HTMLResponse)
async def login_html() -> HTMLResponse:
    """Serve the static login page if present.

    Returns an HTMLResponse containing the contents of login.html.  If the
    file cannot be found, a simple placeholder message is returned instead.
    """
    path = os.path.join(os.path.dirname(__file__), "login.html") if not os.path.isabs("login.html") else "login.html"
    try:
        return FileResponse(path)
    except Exception:
        return HTMLResponse("<html><body><p>Login page not found.</p></body></html>", status_code=404)


@app.get("/signup.html", response_class=HTMLResponse)
async def signup_html() -> HTMLResponse:
    """Serve the static signup page if present.

    Returns an HTMLResponse containing the contents of signup.html.  If the
    file cannot be found, a simple placeholder message is returned instead.
    """
    path = os.path.join(os.path.dirname(__file__), "signup.html") if not os.path.isabs("signup.html") else "signup.html"
    try:
        return FileResponse(path)
    except Exception:
        return HTMLResponse("<html><body><p>Signup page not found.</p></body></html>", status_code=404)
...
class UserStore:
    ...
    def delete_user(self, user_id: str) -> None:
        ...
        This helper deletes the specified user from the store and
        persists the updated list.  It does not remove any associated
        tokens or sessions; callers should ensure those are cleaned
        up separately.
        ...
        # Persist only if the list changed
...
class SessionStore:
    """Persistent session store mapping session tokens to user IDs.

    Previously sessions were stored only in memory, which meant that a
    server restart would invalidate all tokens and cause "Invalid session
    token" errors.  This implementation persists the session mapping to
    a JSON file so that sessions survive restarts.  In production you
    should consider using a more robust session mechanism or JWTs.
    """

    def __init__(self, path: str | None = None) -> None:
        # Determine the path to persist sessions.  Allow override via env
        # variable; otherwise default to 'sessions.json'.
        self.path = path or os.getenv("REPLYFLOW_SESSION_FILE", "sessions.json")
        # Ensure the parent directory exists if specified
        session_dir = os.path.dirname(self.path)
        if session_dir and not os.path.exists(session_dir):
            try:
                os.makedirs(session_dir, exist_ok=True)
            except Exception:
                # Fall back to basename if directory creation fails
                self.path = os.path.basename(self.path)
        # Load any existing sessions from disk; if the file doesn't exist
        # initialise an empty mapping.
            ...
            if os.path.exists(self.path):
                with open(self.path, "r", encoding="utf-8") as fh:
                    ...
                    if isinstance(data, dict):
                        # Filter out non-string keys or values for safety
                        self.sessions = {
                            str(k): str(v) for k, v in data.items()
                        }
        except Exception:
            # If loading fails, fall back to empty sessions.  We do not
            # propagate the exception to avoid crashing at startup.
            self.sessions = {}

    def _save_sessions(self) -> None:
        # Persist the current session mapping to disk.  Ignore errors
        # silently, as a failure to persist should not block operation.
        try:
            with open(self.path, "w", encoding="utf-8") as fh:
...
# Beta user tracking
#
# To selectively enable beta‑only features for specific users, the server
# maintains a simple list of email addresses representing beta testers.  When a
# tester signs up via the beta registration flow, their email is recorded in
# this list.  During the normal account signup process, the email is checked
# against the list; if present, the resulting user record is tagged with
# ``beta_user: True``.  Otherwise the field is omitted or set to False.  This
# mechanism allows the backend and frontend to distinguish beta users from
# regular users.

BETA_USERS_FILE = os.getenv("REPLYFLOW_BETA_USERS_FILE", "beta_users.json")

class BetaUserStore:
    """A lightweight store for tracking beta tester email addresses.

    Beta tester emails are persisted to a JSON file on disk.  The store
    provides methods to append new beta addresses and to check whether a
    given email is registered as a beta tester.  Emails are normalized
    (stripped and lower‑cased) to avoid duplicate entries.
    """

    def __init__(self, path: str = BETA_USERS_FILE) -> None:
        self.path = path
        # Ensure the file exists
        if not os.path.exists(self.path):
            with open(self.path, "w", encoding="utf-8") as fh:
    ...
    def add_beta_email(self, email: str) -> None:
        """Record an email address as belonging to a beta tester.

        The email is normalized before being added.  If the email is already
        present, no changes are made.

        Args:
            email: The email address to record.
        """
    ...
    def is_beta_email(self, email: str) -> bool:
        """Return True if the given email belongs to a beta tester.

        The email is normalized before checking.  Returns False if the list
        cannot be loaded or the email is not present.

        Args:
            email: The email address to check.

        Returns:
            True if the email is registered as a beta tester, False otherwise.
        """
        normalized = (email or "").strip().lower()
        if not normalized:
            return False
...
async def bluesky_oauth_start(
    ...
    session_token = token
    if not session_token:
        login_url = os.getenv("FRONTEND_LOGIN_URL") or "/login.html"
...
async def youtube_upload_video(
    ...
    """
    # Feature flag: if video uploads are disabled globally, or the current
    # account is not flagged as a beta tester, reject requests.  This
    # ensures that only beta users in environments with the feature enabled
    # can upload videos.
    if not ENABLE_VIDEO_UPLOAD or not current_user.get("beta_user", False):
        raise HTTPException(status_code=403, detail="Video upload is restricted to beta testers")
...
def signup(req: SignupRequest) -> dict:
    ...
    # After creating the user, check if the email was registered via the beta
    # signup flow.  If so, mark the account with the beta_user flag.  This
    # additional field will persist in the user store and allow the frontend
    # and backend to conditionally enable beta features for this account.
    beta_flag = False
    try:
        beta_flag = beta_user_store.is_beta_email(email)
    except Exception:
        beta_flag = False
    if beta_flag:
        try:
            user_store.update_user_fields(user["id"], {"beta_user": True})
            # Reflect the flag in the local reference
            user["beta_user"] = True
        except Exception:
            pass
    return {
        ...
        "user_id": user.get("id"),
        # Expose whether the account was tagged as beta in the response
        "beta_user": beta_flag,
    }
...
def signup_get(email: str = Query(...), password: str = Query(...)) -> dict:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=str(exc))
    # Set beta_user flag if this email was registered via the beta flow
    beta_flag = False
    try:
        beta_flag = beta_user_store.is_beta_email(clean_email)
    except Exception:
        beta_flag = False
    if beta_flag:
        try:
...
def me(current_user: dict = Depends(get_current_user)) -> dict:
    ...
    The returned dictionary includes the user ID, email address, plan, and
    whether the account is flagged as a beta tester.  The ``beta_user``
    field defaults to False if not present on the user record.
    """
    return {
        "id": current_user.get("id"),
        "email": current_user.get("email"),
        "plan": current_user.get("plan"),
        "beta_user": bool(current_user.get("beta_user", False)),
    }

# -----------------------------------------------------------------------------
# Account management
#
# Provide an endpoint for users to permanently delete their accounts.  This
# operation requires authentication and removes the user's record, any
# associated tokens, and active sessions.  It returns a simple status
# message on success.

@app.post("/account/delete")
async def account_delete(current_user: dict = Depends(get_current_user)) -> dict:
    """Delete the currently authenticated user's account.

    When invoked by a logged-in user, this endpoint removes the user
    record from the UserStore, deletes any stored access tokens from
    the TokenStore, and invalidates all active sessions for the user.  The
    caller must include a valid Authorization header with a session token.

    Returns:
        A dictionary indicating success.  On error, a 500 status may be
        returned implicitly.
    """
    ...
    # Attempt to delete all stored tokens for this user
    try:
        token_store.delete_tokens_by_user(user_id)  # type: ignore[attr-defined]
    except Exception:
        # Ignore errors when deleting tokens
        pass
    # Remove the user record from the store
    try:
        user_store.delete_user(user_id)
    except Exception:
        pass
    # Invalidate all sessions associated with this user
    try:
        tokens_to_delete = [tok for tok, uid in list(session_store.sessions.items()) if uid == user_id]
        for tok in tokens_to_delete:
            session_store.delete_session(tok)
    except Exception:
        pass
    return {"status": "success"}
