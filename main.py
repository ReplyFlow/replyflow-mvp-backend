...
async def _prepare_facebook_replies(user_id: str, tone: str = "friendly", max_posts: int = 3, max_comments: int = 5) -> list[dict]:
    ...
    async with httpx.AsyncClient() as client:
        for token_info in tokens:
            ...
            # include visitor posts as well. If this request fails or returns no data,
            # attempt the /feed edge as a fallback. If both fail, skip this page.
            feed_data = None
            try:
                ...
                # If the API responded with an error, raise so we can try fallback
                if isinstance(feed_data, dict) and feed_data.get("error"):
                    raise Exception(feed_data.get("error"))
                # Some Pages using New Page Experience may return an empty data list for /posts
                if not feed_data or not feed_data.get("data"):
                    raise Exception("No posts found on /posts edge")
            except Exception:
                # Try the /feed edge as a fallback
                try:
                    alt_feed_resp = await client.get(
                        f"https://graph.facebook.com/v23.0/{page_id}/feed",
                        params={"access_token": request_token, "limit": max_posts},
                        timeout=10,
                    )
                    feed_data = alt_feed_resp.json()
                    # Raise if error present
                    if isinstance(feed_data, dict) and feed_data.get("error"):
                        raise Exception(feed_data.get("error"))
                except Exception:
                    # Could not fetch posts; skip this page
                    continue
