...
class TokenStore:
    ...
    def delete_tokens_by_user(self, user_id: str) -> None:
        """Delete all token records for the given user.

        Args:
            user_id: The user identifier whose tokens should be removed.

        This permanently removes all stored tokens associated with the
        specified user. Call this when deleting a user account to
        ensure that no orphaned credentials remain in the token store.
        """
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("DELETE FROM tokens WHERE user_id = ?", (user_id,))
        except Exception:
            # Ignore deletion errors; missing tokens should not block account deletion
            pass
