...
const PRECACHE_URLS = [
  ...
  '/dashboard.html',
  // Beta signup pages for early access users
  '/beta-signup.html',
...
self.addEventListener('fetch', (event) => {
  ...
  // Only intercept requests that originate from our own domain. This prevents
  // the service worker from hijacking requests to external domains like
  // Netlify’s dashboard or third‑party embeds (e.g. Zoho Forms).
  if (requestURL.origin !== self.location.origin) {
    // Let the network handle cross‑origin requests without caching.
    return;
  }
  event.respondWith(
